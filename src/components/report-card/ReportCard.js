...
const ReportCard = ({ game, level = null }) => {
  ...
  const handleClearReportCard = () =>
    setReportCard(reportCards[`${game}ReportCard`]);

  return (
    <main className="ReportCard">
      <section className="ReportCard-scores">{scoreDisplay}</section>
      <p className="ReportCard-clear" onClick={handleClearReportCard}>
        clear report card
      </p>
    </main>
  );
