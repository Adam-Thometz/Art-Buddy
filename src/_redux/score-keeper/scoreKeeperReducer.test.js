...
describe('Score Keeper reducer', () => {
  ...
  it('should handle adding a point', () => {
    const result = scoreKeeperReducer(addedStudents, updatePoints({ name: 'Jane', instruction: 'add' }));
  ...
  it('should handle removing a point', () => {
    const result = scoreKeeperReducer(addedStudents, updatePoints({ name: 'Jake', instruction: 'subtract' }));
  ...
  it('should not do anything if student points are 0 and the operation is remove', () => {
    const result = scoreKeeperReducer(addedStudents, updatePoints({ name: 'Bob', instruction: 'subtract' }));
